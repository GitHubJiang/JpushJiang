...
public class PushExample {
    ...
    public long sendPushAll() {
        ...
        // 极光推送由v2版本升级到v3版本后，如果不设置这个参数通知推送不了
     ...
     * PushPayload.newBuilder().setPlatform(Platform.ios()).setAudience(Audience.tag_and("tag1",
     * "tag_all"))
     * .setNotification(Notification.newBuilder().addPlatformNotification(IosNotification.newBuilder
     * ().setAlert(ALERT).setBadge(5).setSound("happy.caf").addExtra("from",
     * "JPush").build()).build()).setMessage(Message.content(MSG_CONTENT))
     * .setOptions(Options.newBuilder().setApnsProduction(true).build()).build(); }
     * 
     * public static PushPayload buildPushObject_ios_audienceMore_messageWithExtras() { return
     * PushPayload.newBuilder().setPlatform(Platform.android_ios()).setAudience(Audience.newBuilder(
     * ).addAudienceTarget(AudienceTarget.tag("tag1",
     * "tag2")).addAudienceTarget(AudienceTarget.alias("alias1", "alias2")).build())
     * .setMessage(Message.newBuilder().setMsgContent(MSG_CONTENT).addExtra("from",
     * "JPush").build()).build(); }
     */
